...
<div style="display:table; margin-left:auto; margin-right:auto;">
	...
	while ($row = mysql_fetch_array($results)) {
		...
		<div style="display:table-row; padding:0px;" class="clickInRow">
			...
			<div class="tdiv"><button style="padding:2px;">Submit</button></div>
			<div class="tdiv"><button style="padding:2px;" onClick="duplicate('<?php echo $row['rev'];?>', '<?php echo $row['app'];?>')">Duplicate</button></div>
			<div class="tdiv"><button style="padding:2px;">Delete</button></div>
		</div>
		<?php
	}
	?>
