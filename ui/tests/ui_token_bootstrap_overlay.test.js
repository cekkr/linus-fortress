...
test("token gate hides admin overlay and shows bootstrap action", async (t) => {
  let puppeteer;
  try {
  ...
  try {
    ...
    const adminOverlayHidden = await page.$eval("#admin-overlay", (el) => el.hidden);
    assert.equal(adminOverlayHidden, true);

    await page.waitForSelector("#admin-bootstrap", { visible: true });
  } finally {
    if (browser) {
      await browser.close();
    }
