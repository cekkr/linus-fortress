...
- fortress-cli.py now includes `recipes list|create|apply|plan|seed`, `firewall *`, `sites *`, and `migrations *` helpers in addition to status/api-users/package/backup calls
...
- `py/server.py`: `/status`, `/monitoring/resources`, `/routing`, `/routing/add`, `/routing/{domain}`, `/api-users*`, `/firewall/*`, `/packages/*`, `/recipes*`, `/sites*`, `/migrations*`, `/backup/*`, `/restore`
